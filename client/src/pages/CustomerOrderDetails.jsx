...
const CustomerOrderDetails = () => {
    ...
    const getStatusInfo = (status) => {
        switch (status) {
            ...
            default: return { label: status.toUpperCase(), color: 'text-white', bg: 'bg-white/5', icon: Info };
        }
    };
        ...
        <DashboardLayout title={`Mission Briefing: #${order._id.slice(-6).toUpperCase()}`}>
            <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-1000">
                ...
                >
                    ...
                    <span className="text-[10px] font-black  tracking-normal">Return to Buyer Vault</span>
                </button>
                        ...
                        <div className="relative h-[400px] rounded-[60px] overflow-hidden border border-white/5 bg-[#0A0A0A] group shadow-2xl">
                            ...
                            <div className="absolute bottom-12 left-12 right-12 flex flex-col gap-3">
                                <div className="flex items-center gap-3">
                                    <span className={`px-3 py-1 ${statusInfo.bg} border border-white/5 rounded-full text-[9px] font-black ${statusInfo.color}  tracking-normal flex items-center gap-2`}>
                                        ...
                                        return (
                                            <div key={section.id} className="bg-[#0D0D0D] border border-white/5 rounded-[40px] p-10 flex flex-col md:flex-row md:items-center justify-between gap-8 group hover:border-primary/20 transition-all">
                                                ...
                                                <div className="flex items-center gap-4">
                                                    <div className="px-4 py-2 bg-white/5 rounded-xl border border-white/5 text-[9px] font-black text-white  tracking-normal">
                                                        CONFIRMED
                                                    </div>
                                                </div>
                                            </div>
                                ...
                                <div className="grid grid-cols-1 gap-4">
                                    {service.requirements.map((req, i) => (
                                        <div key={i} className="flex items-center gap-4 p-6 bg-white/[0.01] border border-white/5 rounded-3xl">
                                            ...
                                            <span className="text-sm font-medium text-white leading-relaxed">{req}</span>
                                        </div>
                                    ))}
                                </div>
                            </div>
                    ...
                    <div className="lg:col-span-5 space-y-10 sticky top-12">
                        <div className="bg-[#0A0A0A] border border-white/10 rounded-[56px] p-12 space-y-10 shadow-2xl relative overflow-hidden group">
                            ...
                            <div className="relative z-10 space-y-2">
                                <p className="text-[11px] font-black  text-white tracking-normal">INVESTMENT VALUE</p>
                                ...
                                {pro ? (
                                    <div className="p-8 bg-white/[0.02] border border-primary/20 rounded-[40px] space-y-6">
                                        <div className="flex items-center gap-6">
                                            <div className="w-16 h-16 rounded-full bg-primary/10 border border-primary/20 flex items-center justify-center text-white relative">
                                                <User className="w-8 h-8" />
                                                <div className="absolute -bottom-0.5 -right-0.5 w-5 h-5 bg-green-500 rounded-full border-4 border-[#0A0A0A]"></div>
                                            </div>
                                            <div>
                                                <p className="text-[10px] font-black text-white  tracking-normal">Assigned Specialist</p>
                                                <p className="text-xl font-black text-white  tracking-tight">{pro.name}</p>
                                                <div className="flex items-center gap-2 mt-1">
                                                    <div className="w-1.5 h-1.5 bg-green-500 rounded-full animate-pulse"></div>
                                                    <span className="text-[9px] font-black text-white  tracking-normal">Operator Online</span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="p-8 bg-white/[0.02] border border-white/5 border-dashed rounded-[40px] text-center space-y-4">
                                        ...
                                        <div className="space-y-1">
                                            <p className="text-sm font-black text-white  tracking-tight">Securing Specialist</p>
                                            <p className="text-[9px] font-bold text-white  tracking-normal">Awaiting deployment...</p>
                                        </div>
                                    </div>
                                )}
                            </div>
                            <button 
                                ...
                                className="w-full bg-primary hover:bg-white text-black py-7 rounded-[32px] font-black text-xs  tracking-normal transition-all shadow-xl shadow-primary/20 flex items-center justify-center gap-3 relative z-10"
                            >
