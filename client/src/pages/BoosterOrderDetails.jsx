        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
            ...
            <div className="relative w-full max-w-[500px] bg-[#0A0A0A] border border-white/10 rounded-[48px] p-12 overflow-hidden shadow-2xl animate-in zoom-in-95 duration-300">
                ...
                <div className="relative z-10 text-center space-y-6">
                    ...
                    <h3 className="text-3xl font-black  text-white tracking-tighter">Accept Assignment?</h3>
                    ...
                    <div className="grid grid-cols-2 gap-4 pt-8">
                        <button 
                            onClick={onClose}
                            className="py-5 bg-white/5 hover:bg-white/10 rounded-3xl text-[10px] font-black  tracking-normal transition-all"
                        >
                            Cancel
                        </button>
                        <button 
                            disabled={loading}
                            onClick={onConfirm}
                            className="py-5 bg-primary text-black rounded-3xl text-[10px] font-black  tracking-normal hover:bg-white transition-all shadow-xl shadow-primary/10 disabled:opacity-50"
                        >
...
const BoosterOrderDetails = () => {
    ...
    if (error || !order) return (
        <DashboardLayout title="Access Denied">
            <div className="min-h-[400px] flex flex-col items-center justify-center text-white space-y-6">
                <div className="w-20 h-20 bg-red-500/10 border border-red-500/20 rounded-full flex items-center justify-center">
                    <AlertCircle className="w-10 h-10 text-red-500" />
                </div>
                <h2 className="text-2xl font-black  tracking-tighter">{error}</h2>
                <button 
                    onClick={() => navigate('/pro/dashboard')} 
                    className="px-8 py-3 bg-white/5 border border-white/10 rounded-full text-[10px] font-black  tracking-normal hover:bg-white hover:text-black transition-all"
                >
                    Return to Hub
                </button>
            </div>
    ...
    return (
        <DashboardLayout title={`Mission Registry: #${order._id.slice(-6).toUpperCase()}`}>
            <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-1000">
                ...
                >
                    <div className="w-8 h-8 rounded-full bg-white/5 border border-white/5 flex items-center justify-center group-hover:border-primary/40 group-hover:bg-primary/10 group-hover:text-white">
                        <ChevronLeft className="w-4 h-4 group-hover:-translate-x-0.5 transition-transform" />
                    </div>
                    <span className="text-[10px] font-black  tracking-normal">Return to Operation Center</span>
                </button>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
                    <div className="lg:col-span-7 space-y-16">
                        <div className="relative h-[450px] rounded-[60px] overflow-hidden border border-white/5 bg-[#0A0A0A] group shadow-2xl">
                            ...
                            <div className="absolute bottom-12 left-12 right-12 flex flex-col gap-3">
                                <div className="flex items-center gap-3">
                                    <span className="px-3 py-1 bg-primary/10 border border-primary/30 rounded-full text-[9px] font-black text-white  tracking-normal">
                                        {service?.categorySlug?.replace('-', ' ') || 'Boosting'}
                                    </span>
                                    <span className="text-white">•</span>
                                    <span className="text-[9px] font-black  text-white tracking-normal">{service?.game}</span>
                                </div>
                                <h1 className="text-4xl md:text-5xl font-black  text-white leading-[1.1] tracking-tighter">{service?.title}</h1>
                            </div>
                        </div>
                                            ...
                                            <div key={section.id} className="bg-[#0D0D0D] border border-white/5 rounded-[40px] p-10 flex flex-col md:flex-row md:items-center justify-between gap-8 group hover:border-primary/20 transition-all">
                                                <div className="space-y-2">
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        <p className="text-[11px] font-black  tracking-normal text-white">{section.heading}</p>
                                                    </div>
                                                    <p className="text-2xl font-black text-white  tracking-tight group-hover:text-white transition-colors">
                                                        {displayValue}
                                                    </p>
                                                </div>
                                                <div className="flex items-center gap-4">
                                                    <div className="px-4 py-2 bg-primary/10 rounded-xl border border-primary/20 text-[9px] font-black text-white  tracking-normal">
                                                        MISSION CRITICAL
                                                    </div>
                                                </div>
                                            </div>
                                        ...
                                        {Object.entries(selectedOptions).map(([key, val]) => (
                                            val && (
                                                <div key={key} className="bg-white/[0.02] border border-white/5 rounded-[32px] p-8 space-y-3 group hover:border-primary/20 transition-all">
                                                    <p className="text-[10px] font-black  tracking-normal text-white">
                        ...
                        <div className="space-y-8">
                            ...
                            <div 
                                className="text-white font-medium leading-relaxed prose prose-invert max-w-none text-lg"
                    ...
                    <div className="lg:col-span-5 space-y-10 sticky top-12">
                        <div className="bg-[#0A0A0A] border border-primary/30 rounded-[56px] p-12 space-y-8 relative overflow-hidden group shadow-2xl">
                            ...
                            <div className="relative z-10 space-y-2">
                                <p className="text-[11px] font-black  text-white tracking-normal">MISSION REWARD</p>
                                <div className="flex items-baseline gap-2">
                                    <h2 className="text-6xl font-black text-white tracking-tighter">{formatPrice(order.boosterEarnings || order.price)}</h2>
                                    <span className="text-lg font-bold text-white  tracking-normal">Payout</span>
                                </div>
                            </div>
                            <div className="pt-6 relative z-10">
                                {order.status === 'pending' && !order.pro ? (
                                    <button 
                                        ...
                                        className="w-full bg-primary hover:bg-white text-black py-7 rounded-[32px] font-black text-xs  tracking-normal transition-all shadow-xl shadow-primary/20 active:scale-95 disabled:opacity-50"
                        ...
                        <div className="bg-[#0A0A0A] border border-white/5 rounded-[56px] p-12 space-y-10 shadow-xl">
                            <div className="flex items-center justify-between">
                                <h3 className="text-xl font-black  text-white tracking-tight">Deployment Intel</h3>
                            ...
                            <div className="grid grid-cols-2 gap-8">
                                <div className="space-y-2">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <p className="text-[10px] font-black text-white  tracking-normal">Platform</p>
                                    </div>
                                    <p className="text-sm font-black text-white  tracking-wider">{order.platform || 'Cross-Play'}</p>
                                </div>
                                <div className="space-y-2 text-right">
                                    <div className="flex items-center gap-2 justify-end">
                                        <Globe className="w-3.5 h-3.5 text-primary/40" />
                                        <p className="text-[10px] font-black text-white  tracking-normal">Region</p>
                                    </div>
                                    <p className="text-sm font-black text-white  tracking-wider">{order.region || 'Global'}</p>
                                </div>
                            </div>
                        </div>
                        ...
                        >
                            <div className="flex items-center gap-6">
                                <div className="w-16 h-16 rounded-[24px] bg-primary/10 border border-primary/20 flex items-center justify-center text-white group-hover:scale-110 transition-transform">
                                    <MessageSquare className="w-8 h-8" />
                                </div>
                                <div className="text-left">
                                    <p className="text-[11px] font-black text-white  tracking-normal mb-1">Encrypted Link</p>
                                    <p className="text-lg font-black text-white  tracking-tight">Open Comm-Channel</p>
                                </div>
                            </div>
                            <ChevronRight className="w-6 h-6 text-white group-hover:text-primary transition-all group-hover:translate-x-1" />
                        </button>
                    </div>
                </div>
            </div>
