            ...
            <div className="relative w-full max-w-[900px] max-h-[90vh] bg-[#0A0A0A] border border-white/10 rounded-[48px] overflow-hidden flex flex-col shadow-2xl animate-in fade-in zoom-in-95 duration-300">
                ...
                <div className="flex-1 overflow-y-auto p-8 md:p-12 space-y-12">
                    ...
                    <div className="space-y-4 text-center max-w-2xl mx-auto">
                        <div className="flex items-center justify-center gap-3 mb-2">
                            <span className="px-4 py-1 bg-primary/10 border border-primary/20 text-white text-[10px] font-black  tracking-[0.2em] rounded-full">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div className="space-y-6">
                            <h3 className="text-sm font-black  tracking-[0.2em] text-white flex items-center gap-3">
                            ...
                            <div className="bg-white/[0.02] border border-white/5 rounded-[32px] p-8 space-y-6">
                                {Object.keys(options).length > 0 ? (
                                    ...
                                    <p className="text-sm text-white font-medium italic">Standard configuration</p>
                                )}
                            </div>
                        </div>
                            ...
                            <div className="bg-white/[0.02] border border-white/5 rounded-[32px] p-8 space-y-6">
                                <div className="grid grid-cols-1 gap-6">
                                    <div className="flex items-center justify-between py-3 border-b border-white/5">
                                        ...
                                        <span className="text-xs font-black text-white ">{order?.platform || 'Any'}</span>
                                    </div>
                                    <div className="flex items-center justify-between py-3 border-b border-white/5">
                                        <div className="flex items-center gap-3">
                                        ...
                                        <span className="text-xs font-black text-white ">{order?.region || 'Any'}</span>
                                    </div>
                                    <div className="flex items-center justify-between py-3">
                                        <div className="flex items-center gap-3">
                                            <Clock className="w-4 h-4 text-white/20" />
                                            <span className="text-xs font-bold text-white">Est. Time</span>
                                        </div>
                                        <span className="text-xs font-black text-white ">{service.estimatedCompletionTime || '24h'}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                {service.requirements.map((req, i) => (
                                    <div key={i} className="flex items-center gap-3 p-4 bg-white/[0.01] border border-white/5 rounded-2xl">
                                        ...
                                        <span className="text-xs font-medium text-white">{req}</span>
                                    </div>
                                ))}
                            </div>
                        </div>
                ...
                <div className="p-8 md:p-10 bg-white/[0.02] border-t border-white/5 flex items-center justify-between gap-8 shrink-0">
                    <div className="hidden md:block">
                        <p className="text-[10px] font-black  text-white tracking-[0.2em] mb-1">Booster Payout</p>
                    ...
                    <div className="flex gap-4 flex-1 md:flex-initial">
                        <button 
                            onClick={onClose}
                            className="flex-1 md:w-40 py-5 bg-white/5 hover:bg-white/10 rounded-3xl text-[11px] font-black  tracking-widest transition-all"
                        >
                            Back
                        </button>
                        {order && order.status === 'pending' && !order.pro && (
                            <button 
                                className="flex-1 md:w-64 py-5 bg-primary text-black rounded-3xl text-[11px] font-black  tracking-widest hover:bg-white transition-all shadow-xl shadow-primary/20"
                                onClick={() => {
