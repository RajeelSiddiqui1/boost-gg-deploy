...
const AdminCategories = () => {
 ...
 <p className="text-white text-[10px] font-bold  tracking-[0.2em] mt-1">Hierarchical distribution of services</p>
 </div>
 ...
 className="px-8 py-4 bg-primary hover:bg-[#722AEE] text-white rounded-2xl font-black  tracking-widest text-[11px] transition-all flex items-center justify-center gap-3 shadow-xl shadow-primary/20"
 >
 ...
 {/* Notifications */}
 {notification && (
 <div className={`fixed bottom-10 right-10 z-50 flex items-center gap-3 px-6 py-4 rounded-2xl border animate-in slide-in-from-right duration-300 ${notification.type === 'success' ? 'bg-green-500/10 border-green-500/20 text-green-500' : 'bg-red-500/10 border-red-500/20 text-white'
 }`}>
 {notification.type === 'success' ? <CheckCircle2 className="w-5 h-5" /> : <AlertCircle className="w-5 h-5" />}
 <span className="text-xs font-black  tracking-wider">{notification.message}</span>
 </div>
 )}
 ...
 <th className="px-8 py-6 text-[10px] font-black  tracking-widest text-white/20">Name & Game</th>
 <th className="px-8 py-6 text-[10px] font-black  tracking-widest text-white/20">Slug</th>
 <th className="px-8 py-6 text-[10px] font-black  tracking-widest text-white/20 text-center">Featured</th>
 <th className="px-8 py-6 text-[10px] font-black  tracking-widest text-white/20 text-center">Status</th>
 <th className="px-8 py-6 text-[10px] font-black  tracking-widest text-white/20 text-right">Actions</th>
 </tr>
 </thead>
 ...
 <span className="text-[10px] font-black  tracking-[0.2em] text-white">Updating Stream...</span>
 </div>
 </td>
 </tr>
 ) : categories.length === 0 ? (
 ...
 <span className="text-[10px] font-black  tracking-[0.2em] text-white">No matching categories found</span>
 </td>
 </tr>
 ) : (
 ...
 {category.isFeatured ? (
 <span className="px-3 py-1 bg-yellow-500/10 border border-yellow-500/20 rounded-full text-[8px] font-black text-white  tracking-widest">Featured</span>
 ) : (
 <span className="text-white text-[8px] font-black  tracking-widest">Normal</span>
 )}
 </div>
 </td>
 <td className="px-8 py-6">
 ...
 <span className="text-[9px] font-black  tracking-widest">{category.isActive ? 'Active' : 'Hidden'}</span>
 </button>
 </div>
 </td>
 <td className="px-8 py-6">
 ...
 PAGE <span className="text-white">{currentPage}</span> / {totalPages} — <span className="text-white">{totalItems}</span> RESULTS
 </p>
 <div className="flex items-center gap-2">
 <button
 onClick={() => setCurrentPage(prev => Math.max(prev - 1, 1))}
 disabled={currentPage === 1}
 className="px-6 py-3 bg-white/5 hover:bg-white/10 disabled:opacity-20 rounded-xl text-[10px] font-black text-white  tracking-widest transition-all border border-white/5"
 >
 Prev
 </button>
