...
const BlockEditor = ({ block, onUpdate, onDelete, isSelected, onSelect }) => {
                ...
                return (
                    <div className="space-y-3">
                        <textarea
                            ...
                            onChange={handleContentChange}
                            rows={3}
                            className="w-full bg-transparent border-none focus:outline-none text-white/90 text-lg italic"
    ...
    const renderPreview = () => {
        switch (block.type) {
            case 'heading1':
                return <h1 className="text-4xl md:text-5xl font-black italic uppercase tracking-tighter">{block.content}</h1>;
            case 'heading2':
                return <h2 className="text-2xl md:text-3xl font-black italic uppercase tracking-tight">{block.content}</h2>;
            case 'heading3':
                return <h3 className="text-xl font-black italic uppercase">{block.content}</h3>;
            case 'paragraph':
            ...
            case 'quote':
                return (
                    <blockquote className="border-l-4 border-primary pl-4 my-4">
                        <p className="text-xl font-black italic text-white/90 mb-1">"{block.content}"</p>
...
const BlogBuilder = ({ value, onChange, blogTitle, blogImage }) => {
    ...
    const renderPreviewContent = () => {
        ...
        return blocks.map((block, index) => {
            switch (block.type) {
                case 'heading1':
                    return <h1 key={index} className="text-4xl md:text-5xl font-black italic uppercase tracking-tighter mb-6 mt-8">{block.content}</h1>;
                case 'heading2':
                    return <h2 key={index} className="text-2xl md:text-3xl font-black italic uppercase tracking-tight mb-4 mt-6">{block.content}</h2>;
                case 'heading3':
                    return <h3 key={index} className="text-xl font-black italic uppercase mb-3 mt-4">{block.content}</h3>;
                case 'paragraph':
                ...
                case 'quote':
                    return (
                        <blockquote key={index} className="border-l-4 border-primary pl-6 my-6 py-2">
                            <p className="text-xl font-black italic text-white/90 mb-2">"{block.content}"</p>
        ...
        <div className="flex h-[600px] border border-white/10 rounded-2xl overflow-hidden">
            ...
            <div className="flex-1 bg-[#050505] overflow-y-auto">
                ...
                <div className="p-8 max-w-2xl mx-auto">
                    {/* Blog Title Preview */}
                    {blogTitle && (
                        <h1 className="text-4xl md:text-5xl font-black italic uppercase tracking-tighter mb-6 text-white">
                            {blogTitle}
                        </h1>
                    )}
