...
const PaymentMethodLogo = ({ id, active }) => {
    switch (id) {
        ...
        case 'ideal': return <span className={`text-[10px] font-black  tracking-tighter ${active ? 'text-black' : 'text-[#cc0066]'}`}>iDEAL</span>;
        case 'skrill': return <span className={`text-[10px] font-black  tracking-tight ${active ? 'text-black' : 'text-[#821361]'}`}>SKRILL</span>;
        case 'crypto': return <span className={`text-[10px] font-black  tracking-widest ${active ? 'text-black' : 'text-white'}`}>CRYPTO</span>;
        ...
        return (
            <div className="min-h-screen bg-[#060606] flex flex-col items-center justify-center p-6 text-center space-y-8">
                ...
                <div>
                    ...
                    <p className="text-white text-sm font-bold  tracking-widest max-w-xs mx-auto">Looks like you haven't added anything yet.</p>
                </div>
                <Link to="/" className="px-10 py-5 bg-primary text-black rounded-2xl font-black text-xs  tracking-[0.2em] hover:scale-105 active:scale-95 transition-all shadow-xl shadow-primary/20">Explore Games</Link>
            </div>
        );
    }
        ...
        return (
            <div className="min-h-screen bg-[#060606] flex items-center justify-center p-6 font-['Outfit'] relative overflow-hidden">
                ...
                <div className="max-w-[560px] w-full bg-[#0D0D0D] border border-white/10 rounded-[3rem] p-12 text-center relative z-10 backdrop-blur-3xl shadow-[0_50px_100px_rgba(0,0,0,0.8)]">
                    ...
                    <h2 className="text-5xl font-black  tracking-tighter mb-4 leading-none">Payment Success!</h2>
                    <p className="text-white text-[11px] font-black  tracking-[0.2em] mb-12 leading-relaxed">
                        Order #GG-{Math.floor(Math.random() * 10000)} has been placed.<br/>Check your Discord messages.
                    </p>
                    <div className="grid grid-cols-1 gap-4">
                        <button onClick={() => navigate('/admin')} className="w-full bg-white text-black py-6 rounded-3xl font-black text-xs  tracking-[0.2em] hover:scale-105 transition-all flex items-center justify-center gap-3">
                            Track Order <ArrowRight className="w-4 h-4" />
                        </button>
                        <Link to="/" className="block py-4 text-white/30 hover:text-white font-black text-[10px]  tracking-[0.3em] transition-colors">Return to Marketplace</Link>
                    </div>
                </div>
            </div>
        );
                ...
                <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-8">
                    <div>
                        <div className="flex items-center gap-3 mb-4">
                            ...
                            <span className="text-[10px] font-black text-white  tracking-[0.4em]">Checkout</span>
                        </div>
                        <h1 className="text-5xl md:text-7xl font-black  tracking-tighter leading-none">Finish <span className="text-white">Order</span></h1>
                    </div>

                    {/* Step Indicators */}
                    <div className="flex items-center gap-4 bg-white/[0.02] border border-white/5 p-2 rounded-3xl">
                        {[1, 2].map(s => (
                            <div key={s} className={`flex items-center gap-3 px-6 py-3 rounded-2xl transition-all ${step === s ? 'bg-white/5 text-white' : 'text-white'}`}>
                                <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center text-[10px] font-black ${step === s ? 'border-primary text-white shadow-[0_0_10px_rgba(19,193,0,0.5)]' : 'border-white/10'}`}>{s}</div>
                                <span className="text-[10px] font-black  tracking-[0.2em]">{s === 1 ? 'Contact' : 'Payment'}</span>
                            </div>
                        ))}
                    </div>
                </div>
                    ...
                    <div className="lg:col-span-7 space-y-12">
                        ...
                        {step === 1 && (
                            <div className="space-y-12 animate-fade-in-up">
                                <div>
                                    <h3 className="text-3xl font-black  tracking-tighter mb-2">Who are you?</h3>
                                    <p className="text-white text-[11px] font-black  tracking-widest">Personalize your delivery experience</p>
                                </div>

                                <div className="space-y-8">
                                    <div className="space-y-3">
                                        <label className="text-[10px] font-black  tracking-[0.3em] text-white ml-4">Discord User ID</label>
                                        <div className="relative group">
                                            ...
                                            <input
                                                ...
                                                className="w-full bg-white/[0.03] border border-white/10 rounded-[2.5rem] py-7 pl-20 pr-10 focus:border-primary/50 transition-all outline-none font-black text-white text-sm placeholder:text-white/10  tracking-widest"
                                            />
                                        </div>
                                    </div>

                                    <div className="space-y-3">
                                        <label className="text-[10px] font-black  tracking-[0.3em] text-white ml-4">Email Delivery</label>
                                        <div className="relative group">
                                    ...
                                    {displayMode === 'boosting' && (
                                        <div className="space-y-3 animate-fade-in">
                                            <label className="text-[10px] font-black  tracking-[0.3em] text-white ml-4">Character Name</label>
                                            <input
                                                type="text"
                                                placeholder="ENTER HERO NAME"
                                                value={formData.inGameName}
                                                onChange={(e) => setFormData({ ...formData, inGameName: e.target.value })}
                                                className="w-full bg-white/[0.03] border border-white/10 rounded-[2.5rem] py-7 px-10 focus:border-primary/50 transition-all outline-none font-black text-white text-sm placeholder:text-white/10  tracking-widest"
                                            />
                                        </div>
                                    )}
                                </div>
                                ...
                                <button
                                    ...
                                    className="w-full bg-primary text-black py-7 rounded-[2.5rem] font-black text-sm  tracking-[0.3em] flex items-center justify-center gap-4 transition-all active:scale-95 disabled:opacity-30 shadow-[0_20px_40px_rgba(19,193,0,0.2)] group"
                                >
                        ...
                        {step === 2 && (
                            <div className="space-y-12 animate-fade-in-up">
                                <div className="flex items-center justify-between">
                                    <div>
                                        <h3 className="text-3xl font-black  tracking-tighter mb-2">Payment Choice</h3>
                                        <p className="text-white text-[11px] font-black  tracking-widest">Selected from modal</p>
                                    </div>
                                    <button onClick={handleBack} className="flex items-center gap-2 text-[10px] font-black  tracking-widest text-white/20 hover:text-white transition-colors">
                                    ...
                                    {[
                                        ...
                                        >
                                            <div className="flex justify-between items-start mb-6">
                                                <div className={`w-12 h-10 rounded-xl flex items-center justify-center transition-colors ${formData.paymentMethod === method.id ? 'bg-black text-white' : 'bg-white/5 border border-white/10 text-white'}`}>
                                            ...
                                            <h4 className={`text-xs font-black  tracking-widest mb-1 ${formData.paymentMethod === method.id ? 'text-black' : 'text-white'}`}>{method.name}</h4>
                                            <p className={`text-[9px] font-bold  tracking-tight ${formData.paymentMethod === method.id ? 'text-black/50' : 'text-white'}`}>{method.desc}</p>
                                        </div>
                                    ))}
                                </div>
                                ...
                                <div className="p-8 bg-white/[0.02] border border-white/5 rounded-[2.5rem] flex items-start gap-6">
                                    ...
                                    <div>
                                        <p className="text-[11px] font-black text-white  tracking-widest mb-1">Buy risk-free with BoostGG</p>
                                        <p className="text-[10px] text-white font-medium leading-relaxed  tracking-tight">
                                ...
                                <button
                                    ...
                                    className="w-full bg-primary text-black py-7 rounded-[2.5rem] font-black text-sm  tracking-[0.3em] flex items-center justify-center gap-4 transition-all active:scale-95 shadow-[0_20px_40px_rgba(19,193,0,0.2)] group"
                                >
                    ...
                    <div className="lg:col-span-5">
                        <div className="bg-white text-black rounded-[3rem] p-10 md:p-12 shadow-[0_40px_80px_rgba(0,0,0,0.5)] relative overflow-hidden">
                            ...
                            <div className="relative z-10 flex flex-col h-full">
                                <div className="flex items-center justify-between mb-12">
                                    <div>
                                        <p className="text-[10px] font-black  tracking-widest text-black/40">Your Order</p>
                                        <h3 className="text-xl font-black  tracking-tight">Summary</h3>
                                    </div>
                                        ...
                                        <div key={i} className="flex gap-6 group">
                                            ...
                                            <div className="flex-1 py-1">
                                                <div className="flex items-center gap-2 mb-1">
                                                    <span className="px-2 py-0.5 bg-black/5 rounded text-[8px] font-black  tracking-widest text-black/40">{item.mode || 'Service'}</span>
                                                </div>
                                                <h4 className="text-sm font-black text-black leading-tight  tracking-tight line-clamp-2">{item.title}</h4>
                                ...
                                <div className="pt-8 border-t border-black/5 space-y-6">
                                    {!promoApplied ? (
                                        <div className="space-y-4">
                                            <label className="text-[10px] font-black  tracking-widest text-black/30 ml-2">Promo Code</label>
                                            <div className="flex gap-3">
                                                <div className="relative flex-1">
                                                    ...
                                                    <input
                                                        ...
                                                        className="w-full bg-black/5 border border-transparent rounded-2xl py-4 pl-14 pr-4 text-xs font-black text-black outline-none focus:bg-white focus:border-black/10 transition-all  tracking-widest"
                                                    />
                                                </div>
                                                <button
                                                    onClick={applyPromoCode}
                                                    disabled={promoLoading || !promoCode.trim()}
                                                    className="px-8 py-4 bg-black text-white rounded-2xl font-black text-[10px]  tracking-widest hover:scale-105 transition-all disabled:opacity-20"
                                                >
                                                    {promoLoading ? '...' : 'Apply'}
                                                </button>
                                            </div>
                                        ...
                                        <div className="flex items-center justify-between p-4 bg-green-500/5 border border-green-500/10 rounded-2xl">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <span className="text-[10px] font-black text-green-600  tracking-widest">{promoApplied.code}</span>
                                                    <p className="text-[9px] text-green-600/60 font-bold ">Discount applied</p>
                                                </div>
                                            </div>
                                            <button onClick={() => { setPromoApplied(null); setPromoCode(''); }} className="text-[9px] font-black  tracking-widest text-black/20 hover:text-red-500 transition-colors">Remove</button>
                                        </div>
                                    )}

                                    <div className="space-y-3 pt-4">
                                        <div className="flex justify-between items-center text-[10px] font-black  tracking-widest text-black/40">
                                            <span>Subtotal</span>
                                            <span>{formatPrice(displayTotal)}</span>
                                        </div>
                                        {promoApplied && (
                                            <div className="flex justify-between items-center text-[10px] font-black  tracking-widest text-green-600">
                                        ...
                                        <div className="flex justify-between items-center pt-6 border-t-2 border-black/5">
                                            <span className="text-xs font-black  tracking-[0.3em]">Total Amount</span>
                                            <div className="text-right">
                                                <span className="text-4xl font-black tracking-tighter leading-none">{formatPrice(finalTotal)}</span>
                                                <p className="text-[9px] font-black  tracking-widest text-black/20 mt-1">incl. all fees & tax</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div className="mt-8 p-8 bg-white/[0.02] border border-white/5 rounded-[3rem] flex items-center justify-between">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <p className="text-[10px] font-black  tracking-widest text-white">Need help?</p>
                                    <p className="text-[9px] font-bold  tracking-widest text-white">Our team is online 24/7</p>
                                </div>
                            </div>
                            <button className="px-6 py-3 bg-white/5 hover:bg-white/10 rounded-2xl text-[10px] font-black  tracking-widest transition-all">Open Chat</button>
                        </div>
                    </div>
                </div>
            </div>
