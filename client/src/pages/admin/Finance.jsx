        ...
        <AdminLayout>
            <div className="space-y-8">
                <div className="space-y-1">
                    <h2 className="text-2xl font-black italic uppercase tracking-tight">Finance Control</h2>
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-left">
                            ...
                            <tbody className="divide-y divide-white/5">
                                {loading ? (
                                    <tr><td colSpan="5" className="px-8 py-20 text-center text-white/20 font-black italic animate-pulse">Auditing Vaults...</td></tr>
                                ) : payouts.length === 0 ? (
                                    <tr><td colSpan="5" className="px-8 py-20 text-center text-white/20 font-black italic">No payout requests pending</td></tr>
                                ) : (
                                    payouts.map((payout) => (
                                        <tr key={payout._id} className="hover:bg-white/[0.01] transition-colors group">
                                            ...
                                            <td className="px-8 py-6">
                                                <p className="text-sm font-black italic text-green-500">{formatPrice(payout.amount)}</p>
                                            </td>
                                            <td className="px-8 py-6">
                                            ...
                                            <td className="px-8 py-6 text-right space-x-2">
                                                ...
                                                {payout.status !== 'pending' && (
                                                    <span className="text-[8px] font-bold text-white/20 uppercase italic">Processed by {payout.processedBy?.name || 'System'}</span>
                                                )}
                                            </td>
                                        </tr>
                                    ))
