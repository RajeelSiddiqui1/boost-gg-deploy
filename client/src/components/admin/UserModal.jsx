...
const UserModal = ({ user, onClose, onUpdate }) => {
 ...
 <span className="text-xs font-bold  text-white tracking-wider">@{user.username || 'unknown'}</span>
 {user.role === 'admin' && (
 <span className="px-2 py-0.5 bg-red-500/10 text-white border border-red-500/20 rounded text-[9px] font-black  tracking-widest">
 {user.email === 'admin@boostgg.com' ? 'Super Admin' : 'Admin'}
 </span>
 )}
 </div>
 </div>
 <div className={`px-3 py-1 rounded-full text-[10px] font-black  tracking-widest border ${user.isActive
 ...
 <span className="text-xs font-bold  text-white">Joined</span>
 </div>
 ...
 <p className="text-sm font-bold text-white">${user.earnings || 0}</p>
 </div>
 </div>
 ...
 <h4 className="text-[10px] font-black  text-white tracking-widest">PRO Application Review</h4>
 </div>
 <div className="grid grid-cols-1 gap-3">
 <div className="p-3 bg-black/40 rounded-lg">
 <p className="text-[8px] font-black  text-white mb-1">Discord</p>
 <p className="text-xs font-bold text-white">{user.proApplication.discord}</p>
 </div>
 <div className="p-3 bg-black/40 rounded-lg">
 <p className="text-[8px] font-black  text-white mb-1">Games</p>
 <p className="text-xs font-bold text-white">{user.proApplication.games}</p>
 </div>
 <div className="p-3 bg-black/40 rounded-lg">
 <p className="text-[8px] font-black  text-white mb-1">Experience</p>
 ...
 disabled={loading}
 className="flex-1 py-3 bg-primary text-black text-[10px] font-black  rounded-xl hover:bg-white transition-all disabled:opacity-50"
 >
 Approve as PRO
 </button>
 <button
 ...
 disabled={loading}
 className="py-3 px-6 bg-red-500/10 text-red-500 border border-red-500/20 text-[10px] font-black  rounded-xl hover:bg-red-500 hover:text-white transition-all disabled:opacity-50"
 >
 Reject
 </button>
 </div>
 ...
 <div>
 <h4 className="text-xs font-black  text-white tracking-widest mb-4">Danger Zone</h4>

 {confirmDelete ? (
 <div className="bg-red-500/10 border border-red-500/20 rounded-xl p-4 flex items-center justify-between">
 <p className="text-xs font-bold text-white">Permantly delete data?</p>
 <div className="flex gap-2">
 <button
 onClick={() => setConfirmDelete(false)}
 className="px-3 py-1.5 bg-transparent hover:bg-white/5 text-white/60 text-[10px] font-bold  rounded-lg transition-colors"
 >
 Cancel
 </button>
 <button
 ...
 disabled={loading}
 className="px-3 py-1.5 bg-red-500 hover:bg-red-600 text-white text-[10px] font-bold  rounded-lg transition-colors shadow-lg shadow-red-500/20"
 >
 {loading ? '...' : 'Confirm'}
 </button>
 </div>
 ...
 <span className="text-xs font-bold  tracking-wider">Delete User</span>
 </button>
 )}

 {user.email === 'admin@boostgg.com' && (
 <p className="text-[10px] text-center text-white mt-2 font-bold  tracking-wider">
 ...
 <button
 onClick={onClose}
 className="w-full py-3 bg-white/5 hover:bg-white/10 text-white font-bold  text-xs rounded-xl transition-all border border-white/5 hover:border-white/10"
 >
 Close
 </button>
 </div>
