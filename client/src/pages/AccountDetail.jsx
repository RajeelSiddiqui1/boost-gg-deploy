...
const RelatedAccountCard = ({ account }) => {
  ...
  return (
    <div 
      ...
      className="group relative bg-[#1a1a1a] rounded-[20px] overflow-hidden flex flex-col transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl h-full border border-white/5 hover:border-white/10 cursor-pointer"
    >
      {/* Image Section */}
      <div className="relative h-[200px] w-full overflow-hidden bg-gradient-to-b from-[#222] to-[#1a1a1a]">
        ...
        {account.gameId?.icon && (
          <img 
            ...
            className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 h-24 object-contain transition-transform duration-500 group-hover:scale-110 drop-shadow-[0_10px_20px_rgba(0,0,0,0.8)] z-10 opacity-60 group-hover:opacity-100" 
            alt="" 
          />
        )}
      ...
      <div className="p-5 flex flex-col flex-grow">
        <div className="flex items-center gap-2 mb-2">
          <span className="text-[10px] font-black text-white/40 tracking-wider uppercase">{account.region}</span>
          <span className="w-1 h-1 bg-white/20 rounded-full"></span>
          <span className="text-[10px] font-black text-white/40 tracking-wider uppercase">{account.server}</span>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2 mb-4">
          <div className="bg-white/[0.03] border border-white/[0.05] rounded-xl p-2 flex items-center gap-2">
            ...
            <span className="text-[10px] font-bold text-white truncate">{account.rank || 'N/A'}</span>
          </div>
          <div className="bg-white/[0.03] border border-white/[0.05] rounded-xl p-2 flex items-center gap-2">
            <Layers className="w-3.5 h-3.5 text-primary" />
            <span className="text-[10px] font-bold text-white truncate">{account.specifications?.skinsCount || 0} Skins</span>
          </div>
        </div>

        <div className="mt-auto pt-4 border-t border-white/5 flex items-center justify-between">
          <span className="text-xl font-bold text-white tracking-tighter">{formatPrice(account.price)}</span>
          <button 
            onClick={handleAddToCart}
            className="w-10 h-10 bg-primary/10 border border-primary/20 rounded-xl flex items-center justify-center text-primary hover:bg-primary hover:text-black transition-all"
          >
