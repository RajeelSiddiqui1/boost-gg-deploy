      ...
      <div className="space-y-8">
        <div className="flex items-center justify-between">
          <button 
            ...
            className="flex items-center gap-2 text-[10px] font-black  text-white/40 hover:text-white transition-colors tracking-widest"
          >
          ...
          <button 
            ...
            className="flex items-center gap-3 px-6 py-3 bg-white/5 border border-white/10 rounded-2xl text-[10px] font-black  text-primary hover:bg-primary/10 transition-all tracking-widest"
          >
          ...
          <div className="flex flex-col h-full min-h-[500px]">
            <div className="mb-12">
              <h2 className="text-4xl font-black  tracking-tighter mb-4 flex items-center gap-4">
                Deployment Nexus
                <span className="text-xs px-3 py-1 bg-primary/10 text-white border border-primary/20 rounded-full font-bold tracking-widest">#{order._id.slice(-6).toUpperCase()}</span>
              </h2>
              
              <div className="flex p-2 bg-white/5 rounded-3xl border border-white/10 w-fit">
                <button 
                  onClick={() => handleTabChange('bidders')}
                  className={`px-8 py-4 rounded-2xl text-[10px] font-black  tracking-widest transition-all ${activeBidTab === 'bidders' ? 'bg-primary text-black' : 'text-white/40 hover:text-white'}`}
                >
                  Negotiators ({bids.filter(b => b.type === 'bid').length})
                </button>
                <button 
                  onClick={() => handleTabChange('claimers')}
                  className={`px-8 py-4 rounded-2xl text-[10px] font-black  tracking-widest transition-all ${activeBidTab === 'claimers' ? 'bg-primary text-black' : 'text-white/40 hover:text-white'}`}
                >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {filteredBids.length === 0 ? (
                <div className="col-span-full flex flex-col items-center justify-center text-white font-black  tracking-[0.2em] border-2 border-dashed border-white/5 rounded-[48px] py-32 space-y-6">
                ...
                filteredBids.map((bid) => (
                  ...
                  >
                    ...
                    <div className="bg-primary/10 border border-primary/20 rounded-full px-4 py-2 mb-8">
                       <p className="text-xl font-black text-white">{formatPrice(bid.bidAmount)}</p>
                    </div>
                    ...
                    <h4 className="text-2xl font-black text-white  tracking-tight mb-2 group-hover:text-white transition-colors">{bid.proId?.name}</h4>
                    <div className="flex items-center gap-2 justify-center mb-10 bg-white/5 px-4 py-1.5 rounded-full border border-white/5">
                      ...
                      <span className="text-[10px] font-black text-white  tracking-widest">{bid.proId?.rating || 5.0} Rating</span>
                    </div>

                    <div className="w-full pt-6 border-t border-white/5">
                       <button className="w-full py-4 rounded-2xl bg-white/5 text-[9px] font-black  tracking-[0.2em] text-white/40 group-hover:bg-primary group-hover:text-black transition-all flex items-center justify-center gap-2">
            ...
            <div className="bg-[#0A0A0A] border border-white/10 rounded-[64px] w-full max-w-4xl overflow-hidden relative animate-in fade-in zoom-in duration-300">
               <div className="relative h-64 overflow-hidden">
                  ...
                  <div className="absolute bottom-10 left-12 flex items-end gap-8">
                     ...
                     <div className="mb-2">
                        <p className="text-[10px] font-black text-white  tracking-[0.3em] mb-2">{order.serviceId?.gameId?.name || 'Tactical Sector'}</p>
                        <h3 className="text-4xl font-black text-white  tracking-tighter">{order.serviceId?.title}</h3>
                     </div>
                  </div>
               ...
               <div className="p-12 md:p-20 grid grid-cols-1 md:grid-cols-2 gap-16">
                  <div className="space-y-10">
                     <div>
                        <p className="text-[10px] font-black text-white  tracking-widest mb-4">Operation Parameters</p>
                        <div className="bg-white/5 border border-white/10 rounded-3xl p-6 space-y-4">
                           <div className="flex justify-between items-center">
                              <span className="text-[10px] font-bold text-white ">Initial Funding</span>
                           ...
                           <div className="flex justify-between items-center">
                              ...
                              <span className="text-[10px] font-black text-white ">{order.platform}</span>
                           </div>
                           <div className="flex justify-between items-center">
                              <span className="text-[10px] font-bold text-white ">Region</span>
                              <span className="text-[10px] font-black text-white ">{order.region}</span>
                           </div>
                        </div>
                     </div>
                     ...
                     {order.selectedOptions && Object.keys(order.selectedOptions).length > 0 && (
                        <div>
                           <p className="text-[10px] font-black text-white  tracking-widest mb-4">Tactical Options</p>
                           <div className="flex flex-wrap gap-2">
                              {Object.entries(order.selectedOptions).map(([key, val]) => (
                                 <span key={key} className="px-4 py-2 bg-white/5 border border-white/5 rounded-xl text-[10px] font-black text-white ">
                  ...
                  <div className="space-y-10">
                     <div>
                        <p className="text-[10px] font-black text-white  tracking-widest mb-4">Customer Briefing</p>
                        <div className="flex items-center gap-4 bg-white/5 border border-white/10 rounded-[32px] p-6">
                           <div className="w-12 h-12 rounded-2xl bg-primary/10 flex items-center justify-center text-white">
                              <User className="w-6 h-6" />
                           </div>
                           <div>
                              <p className="text-[10px] font-black text-white ">{order.userId?.name}</p>
                              <p className="text-[9px] font-bold text-white ">Deployment Requester</p>
                           </div>
                        </div>
                     </div>

                     <div>
                        <p className="text-[10px] font-black text-white  tracking-widest mb-4">Contact Protocol</p>
              ...
              <div className="flex-1 overflow-y-auto custom-scrollbar p-12 md:p-20">
                <div className="grid grid-cols-1 md:grid-cols-12 gap-16">
                  ...
                  <div className="md:col-span-5 flex flex-col items-center text-center">
                    ...
                    <h3 className="text-5xl font-black text-white  tracking-tighter leading-none mb-4">{selectedProDetail.proId?.name}</h3>
                    <p className="text-[12px] font-black text-white  tracking-[0.4em] italic">Special Operations Pro</p>
                    ...
                    <div className="grid grid-cols-1 w-full gap-4">
                      <div className="bg-white/5 rounded-[32px] p-8 border border-white/5 flex flex-col items-center">
                        ...
                        <p className="text-[10px] font-black text-white  mb-2">Time on Platform</p>
                      ...
                      <div className="bg-white/5 rounded-[32px] p-8 border border-white/5 flex flex-col items-center">
                        ...
                        <p className="text-[10px] font-black text-white  mb-2">Missions Done</p>
                  ...
                  <div className="md:col-span-7 space-y-12">
                    <div>
                      <p className="text-[11px] font-black  text-white tracking-widest mb-6 flex items-center gap-3">
                      ...
                      <div className="flex flex-wrap gap-3">
                        {selectedProDetail.proId?.specializedGames?.length > 0 ? (
                          selectedProDetail.proId.specializedGames.map(game => (
                            <span key={game._id} className="px-6 py-3 bg-white/5 text-white text-[10px] font-black  rounded-2xl border border-white/10 hover:border-primary/50 transition-colors">
                              {game.title} Master
                            </span>
                          ))
                        ) : (
                          <p className="text-[10px] font-bold text-white  italic">No specialized sectors registered</p>
                        )}
                      </div>
                    </div>

                    <div className="space-y-6">
                      <p className="text-[11px] font-black  text-white tracking-widest ml-4">Candidate Tactical Statement</p>
                      <div className="bg-white/5 border border-white/10 rounded-[40px] p-10 text-lg font-bold text-white leading-relaxed italic relative">
                    ...
                    <div className="pt-8">
                      <div className="flex items-center justify-between p-8 bg-primary/5 border border-primary/20 rounded-[40px] mb-8">
                        <div>
                          <p className="text-[10px] font-black text-white  mb-1">Deployment Budget</p>
                          <p className="text-3xl font-black text-white">{formatPrice(selectedProDetail.bidAmount)}</p>
                        </div>
                        <div className="text-right">
                          <p className="text-[10px] font-black text-white  mb-1">Platform Rating</p>
                      ...
                      <button
                        ...
                        className="w-full py-8 bg-primary text-black rounded-[40px] font-black text-sm  tracking-[0.3em] hover:bg-white transition-all shadow-2xl shadow-primary/40 flex items-center justify-center gap-4 group active:scale-95"
                      >
